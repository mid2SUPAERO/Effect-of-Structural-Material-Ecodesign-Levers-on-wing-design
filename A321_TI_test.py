...
surf_dict = {
    ...
    "E": 120e9,  # [Pa] Young's modulus
    "G": (
        120e9 / 2 / 1.33
    ),  # [Pa] shear modulus (calculated using E and the Poisson's ratio here)
    "yield": (880e6 / 1.5),  # [Pa] allowable yield stress
    "mrho": 4.50e3,  # [kg/m^3] material density
    ...
    "Wf_reserve": 3500,  # [kg] reserve fuel mass
}
...
indep_var_comp.add_output(
    "v", val=np.array([0.78 * 295.1, 0.53 * 340.294]), units="m/s"
)
indep_var_comp.add_output("alpha", val=0.0, units="deg")
indep_var_comp.add_output("alpha_maneuver", val=0.0, units="deg")
indep_var_comp.add_output("Mach_number", val=np.array([0.78, 0.53]))
indep_var_comp.add_output(
    "re",
    val=np.array(
        [
            0.3482675 * 295.1 * 0.78 * 1.0 / (1.440044 * 1e-5),
            1.225 * 340.294 * 0.53 * 1.0 / (1.81206 * 1e-5),
        ]
    ),
    units="1/m",
)
indep_var_comp.add_output("rho", val=np.array([0.3482675, 1.225]), units="kg/m**3")
indep_var_comp.add_output("CT", val=0.5 / 3600, units="1/s")
indep_var_comp.add_output("R", val=5e6, units="m")
indep_var_comp.add_output("W0", val=57200 + surf_dict["Wf_reserve"], units="kg")
...
indep_var_comp.add_output("fuel_mass", val=15500, units="kg")
